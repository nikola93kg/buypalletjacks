...
export default function LocationsPanel({
  ...
  return (
    <div
      className={`flex flex-col h-full ${compact ? "min-h-[280px] max-h-[420px]" : "min-h-[400px]"}`}
    >
      {/* Panel header */}
      <div className={`p-4 border-b ${dk ? 'border-[#1E3A8A]/30' : 'border-[#E2E8F0]'}`}>
        ...
        <div className="relative">
          ...
          <input
            ...
            className={`w-full border text-sm rounded-lg pl-8 pr-3 py-2.5 focus:outline-none focus:border-[#3B82F6] transition-colors ${dk ? 'bg-[#0A1628] border-[#1E3A8A]/40 text-white placeholder:text-[#475569]' : 'bg-white border-[#CBD5E1] text-[#0F172A] placeholder:text-[#94A3B8]'}`}
      ...
      <div className="flex-1 overflow-y-auto overscroll-contain">
        {showAll && (
          <div className="p-5 flex flex-col items-center justify-center h-full text-center gap-3">
            <div className={`w-10 h-10 rounded-full ${dk ? 'bg-[#1E3A8A]' : 'bg-[#DBEAFE]'} flex items-center justify-center`}>
              <MapPin size={18} className={dk ? 'text-[#60A5FA]' : 'text-[#1D4ED8]'} />
            </div>
            <p className={`${dk ? 'text-[#94A3B8]' : 'text-[#64748B]'} text-sm`}>
              Click a highlighted state on the map to see nearby pickup locations.
            </p>
            <p className={`${dk ? 'text-[#475569]' : 'text-[#94A3B8]'} text-xs`}>
        ...
        {!showAll && filtered.length === 0 && (
          <div className="p-5 text-center">
            <p className="text-[#64748B] text-sm">
              ...
              <a href="tel:+18005555555" className={`${dk ? 'text-[#60A5FA]' : 'text-[#1D4ED8]'} hover:underline`}>
                call us
              </a>{" "}
        ...
        {filtered.length > 0 && (
          <ul className="p-2 flex flex-col gap-1.5" role="list">
            {filtered.map((loc, i) => (
      ...
      {filtered.length > 0 && (
        <div className={`p-4 border-t ${dk ? 'border-[#1E3A8A]/30' : 'border-[#E2E8F0]'}`}>
          <Link
            href="/locations"
            className={`flex items-center justify-center gap-2 ${dk ? 'text-[#60A5FA] hover:text-white' : 'text-[#1D4ED8] hover:text-[#1E3A8A]'} text-sm font-medium transition-colors`}
          >
...
function LocationCard({
  ...
  const dk = theme === 'dark';
  return (
    <div className={`${dk ? 'bg-[#0A1628] hover:bg-[#0D1E38] border-[#1E3A8A]/30 hover:border-[#2563EB]/50' : 'bg-white hover:bg-[#F8FAFC] border-[#E2E8F0] hover:border-[#BFDBFE]'} border rounded-lg p-3 transition-colors group`}>
      <div className="flex items-start justify-between gap-2">
        ...
        <a
          ...
          className={`flex-shrink-0 ${dk ? 'text-[#64748B] hover:text-[#60A5FA]' : 'text-[#94A3B8] hover:text-[#1D4ED8]'} transition-colors p-1`}
